...
main(void)
{
    ...
    for(;;)
    {
        ...
        for(ulMotor = 0; ulMotor < 2; ulMotor++)
        {
            ...
            //
            bButtonIsPressed = !PushButtonGetDebounced((tButton)ulMotor);
            bBumperIsPressed = !BumpSensorGetDebounced((tBumper)ulMotor);
            ...
            switch(sMotorState[ulMotor])
            {
                ...
                case STATE_STOPPED:
                {
                    ...
                    if(bButtonIsPressed && !bButtonWasPressed[ulMotor])
                    {
                        ...
                        //
                        bReverse[ulMotor] ^= 1;
                        MotorDir((tSide)ulMotor, (tDirection)bReverse[ulMotor]);

                        //
                        // Set the motor duty cycle to 50%.
                        // Duty cycle is specified as 8.8 fixed point.
                        //
                        MotorSpeed((tSide)ulMotor, 90 << 8);

                        //
                        // Start the motor running
                        //
                        MotorRun((tSide)ulMotor);
