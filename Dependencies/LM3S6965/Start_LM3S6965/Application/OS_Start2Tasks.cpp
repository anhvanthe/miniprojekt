...
public:
  CThread(char* s, unsigned int p, unsigned int t):sName(s), Priority(p), Timeout(t) {
    ...
    if (pTaskStack != NULL) {
      OS_CreateTaskEx(this, sName, p, CThread::run, reinterpret_cast<OS_STACKPTR void*>(pTaskStack), 256u, 2u, reinterpret_cast<void*>(Timeout));
    }
  }
...
private:
  ...
  static void run(void* t) {
    while (1) {
      OS_Delay(reinterpret_cast<unsigned long>(t));
    }
  }
};
